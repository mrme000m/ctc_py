...
class TotalMarginCalculationType(IntEnum):
    MAX = 0
    SUM = 1
    NET = 2


class LimitedRiskMarginCalculationStrategy(IntEnum):
    ACCORDING_TO_LEVERAGE = 0
    ACCORDING_TO_GSL = 1
    ACCORDING_TO_GSL_AND_LEVERAGE = 2


class StopOutStrategy(IntEnum):
...
class SwapCalculationType(IntEnum):
    PIPS = 0
    PERCENTAGE = 1
    POINTS = 2
...
class MinCommissionType(IntEnum):
    CURRENCY = 1
    QUOTE_CURRENCY = 2


class SymbolDistanceType(IntEnum):
...
class ChangeBalanceType(IntEnum):
    """All cash movement operation types (ProtoOAChangeBalanceType)."""
    BALANCE_DEPOSIT = 0
    BALANCE_WITHDRAW = 1
